...
class BaseViewModelMapper(Generic[ModelType, ViewModelType]):
    """
    Base class for mapping between domain entities and view entities.
    """
    ...
    def to_view_models(self, models: List[ModelType]) -> List[ViewModelType]:
        """
        Convert a list of domain entities to view entities.
        
        Args:
            models: List of domain entities to convert
            
        Returns:
            List of converted view entities
        """
        try:
            return [self.to_view_model(model) for model in models]
        except Exception as e:
            self.logger.error(f"Error converting entities to view entities: {str(e)}")
            raise
    ...
    def to_models(self, view_models: List[ViewModelType]) -> List[ModelType]:
        """
        Convert a list of view entities to domain entities.
        
        Args:
            view_models: List of view entities to convert
        ...
        try:
            ...
            self.logger.error(f"Error converting view entities to entities: {str(e)}")
            raise 